 ...
 * @create 2023/5/30 8:46
 */
@RestController
@RequestMapping("hjSet")
@Slf4j
...
//        KafkaUtils.kafkaSendData(jsonObject,kafkaProducer,topic);
    ...
    @ApiOperation(value = "中控请求侧板转序配送请求上传", notes = "焊接")
    @PostMapping("/sidePanelRotation")
    public Result sidePanelRotationUpload(@MyLog @RequestBody JSONObject jsonObject){
        String taskId = Result.getTaskId(jsonObject);
//        ForwardUtils.postAsyncMOM(momIp,"SidePanelRequest",jsonObject);
        ForwardUtils.postAsyncZK(materialsIp,"lesRequestIssue",jsonObject);
        return Result.success("version",taskId,"ok");
    }
}
